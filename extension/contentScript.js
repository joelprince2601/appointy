...
// Try to get auth token from page on load (if on main app domain)
// This runs when the content script loads on any page
(async () => {
  try {
    if (window.location.hostname.includes('localhost') || 
        window.location.hostname.includes('synapse') ||
        window.location.hostname.includes('supabase')) {
      const token = AuthHelper.getTokenFromPage();
      if (token) {
    ...
    dialog.querySelectorAll('.synapse-dialog-btn:not(:disabled)').forEach((btn) => {
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          if (response && response.success) {
            showToast(`${type === 'page' ? 'Page' : 'Selected text'} saved to database!`);
            resolve({ success: true, type });
          } else {
            showToast(response?.error || 'Failed to save. Please try again.');
